...
const baseRegisterUserSchema = z
  .object({
...
const userRoleEasyDraftUserOptions: DropdownOpts = [{ label: roleTypes.USER, value: roleTypes.USER }];
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (validation.data.forcePasswordChange) {
    ...
    // Call the new API endpoint to send the email with the login code.
    const emailRes = await callAPI<GenericAPI>(
      request,
      '/api/auth/sendLoginCodeEmail', // NEW API ENDPOINT
      { 
    ...
    if (!emailRes.success) {
      return json({
        status: 'api error',
        message: 'User added successfully, but email with login code failed to send.',
        userId: null,
        errors: null,
        fieldErrors: null,
      });
    ...
    return json({
      status: 'success',
      message: 'User account created and login code sent to user via email.',
    ...
    // --- Start of existing logic (when forcePasswordChange is false) ---
    ...
    if (!validation.data.password) {
      const resetRes = await callAPI<GenericAPI>(
        request,
        '/api/auth/forgetPassword',
        { email: validation.data.email, isInvite: true },
        'POST'
      );
      if (!resetRes.success) {
        return json({
  ...
  return (
    <>
      ...
      <Form ref={formRef} method='post' className='flex flex-col gap-3'>
        ...
        <div>
        <SwitchToggle enabled={forcePasswordChange} setEnabled={setForcePasswordChange} />
        <label>   Require password change at next login</label> </div>
