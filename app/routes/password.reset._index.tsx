...
import { z } from 'zod';
import { type LoaderFunctionArgs, json, type ActionFunctionArgs, type MetaFunction } from '@remix-run/node';
...
export const meta: MetaFunction = () => {
  return [
    { title: 'Easy Dairy | Reset Password' },
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const errorMessage = 'The Reset Link is Expired or Invalid';
  if (!code || !email) {
    return json({
      status: 'code_invalid',
      error: errorMessage,
  ...
  try {
    ...
    return json({ status: 'success', error: '', code, email });
  } catch (error) {
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (res.status !== 200) {
      return json({ status: 'failure', errors: { code: 'Fail to reset password' } });
    }
    return json({ status: 'success', errors: {} });
  } catch (error) {
    console.log('error reset password', error);
    return json({ status: 'failure', errors: { code: 'Something went wrong. Fail to reset password' } });
  }
}
