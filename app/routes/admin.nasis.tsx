...
const csvColumnSchema = z.object({
  ...
  VersionNo: z.string().nullable(),
  NationalID: z.string().nullable(),
  CountryCode: z.string().nullable(),
  Herdbook: z.string().nullable(),
  LocalBullID: z.string().nullable(),
  DateOfBirth: z.string().nullable(),
  Breed: z.string().nullable(),
  SireNationalID: z.string().nullable(),
  DamNationalID: z.string().nullable(),
  MGSNationalID: z.string().nullable(),
  Name: z.string().nullable(),
  PrimaryID: z.string().nullable(),
  BullID: z.string().nullable(),
  OwnerCode: z.string().nullable(),
  InternationalID: z.string().nullable(),
  PTSamplingCode: z.string().nullable(),
  DateFirstSemen: z.string().nullable(),
  GeneticCodes: z.string().nullable(),
  NASISActive: z.string().nullable(),
  CommonName1: z.string().nullable(),
  CommonName2: z.string().nullable(),
  DateSexedSemen: z.string().nullable(),
});
type NASISHistory = {
  id: string;
  filename: string;
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (action && action === 'rollback'){
    ...
    apiProps.body = {id: formData.get('id'), type: 'NASIS'}
    apiProps.method = 'PUT'
  } else {
    apiProps.url = '/api/dev/admin/import/NASIS'
...
function UpdateNASISAnimal() {
  ...
  const validateCsv = (csvContent: string) => {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (rowData.length > 0 && rowData.length < header.length) {
  ...
  return (
    <>
      ...
      />
      <h2 className='text-xl lg:text-2xl mb-5'>Update NASIS Animal</h2>
      <Form ref={formRef} method='post' encType='multipart/form-data' className='max-w-xl flex gap-3'>
        <Input type='file' label='CSV' name='csv' error={csvError ?? ''} onChange={handleFileChange} className='' />
        <Button
          type='submit'
      ...
      <div className='overflow-auto w-full mt-10'>
        <h2 className='text-xl lg:text-2xl mb-5'>NASIS History</h2>
                ...
                <tr key={d.id} className='border-t'>
                  ...
                  <td className='truncate max-w-[15ch] md:max-w-xs'>
                    <HoverCard openDelay={100} closeDelay={100}>
                      ...
                      <HoverCard.Content className='rounded p-4'>
                        <ul className='list-disc list-inside'>
                          {d.animals.map((animal) => (
                            <li>
                              {animal}
                            </li>
                          ))}
...
export default UpdateNASISAnimal;
